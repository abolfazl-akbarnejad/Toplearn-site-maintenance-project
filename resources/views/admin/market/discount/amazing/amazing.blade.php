                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            @foreach ($amazings as $key => $amazing)
                                <tr>
                                    <th>{{ $key + 1 }}</th>
                                    <th>{{ $amazing->product->name }}</th>
                                    <th>{{ $amazing->percentage }}%</th>
                                    <td>{{ jalalidate($amazing->start_date) }}</td>
                                    <td>{{ jalalidate($amazing->end_date) }}</td>
                                    <td class="width-16-rem text-left">
                                        <a href="{{ route('admin.market.discount.amazingSale.edit', $amazing->id) }}"
                                            class="btn btn-primary btn-sm"><i class="fa fa-edit"></i>
                                            ویرایش</a>
                                            <form action="{{ route('admin.market.discount.amazingSale.destroy', $amazing->id) }}"
